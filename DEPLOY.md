...
2. **New token** → Type: **Write** → name it e.g. `bhashabridge-deploy` → **Create**.
...
export APP_API_KEY=BHASHABRIDGE_SECRET_KEY     # the X-API-Key clients must send
export ALLOWED_ORIGINS="*"                     # tighten to your Vercel URL later
python deploy/deploy_hf_space.py
```
This creates `https://huggingface.co/spaces/<you>/bhashabridge-backend`, sets all
config as Space variables/secrets, and uploads the code. The Space then builds
(~a few minutes) and the app is served at:
```
https://<you>-bhashabridge-backend.hf.space
```
Check health: `GET https://<you>-bhashabridge-backend.hf.space/api/v1/health`
...
1. https://vercel.com → **Add New… → Project** → import the `BhashaBridge` repo.
2. **Root Directory**: `frontend`  (Framework preset auto-detects **Vite**).
3. **Environment Variables**:
   | Name | Value |
   |------|-------|
   | `VITE_API_BASE_URL` | `https://<you>-bhashabridge-backend.hf.space` |
   | `VITE_API_KEY` | `BHASHABRIDGE_SECRET_KEY` (same as `APP_API_KEY` above) |
4. **Deploy**. Vercel gives you a URL like `https://bhashabridge.vercel.app`.
...
```bash
export ALLOWED_ORIGINS="https://bhashabridge.vercel.app"
python deploy/deploy_hf_space.py
```
...
- SQLite (`bhashabridge.db`) and the FAISS index are ephemeral on the free tier
